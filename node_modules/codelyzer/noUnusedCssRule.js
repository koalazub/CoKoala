...
var lang = require('cssauron')({
  ...
  class: function (node)
    {
      var classBindings = (node.inputs || [])
        ...
        .join(' ');
      var classAttr = node.attrs.find(function (a)
      {
        return a.name.toLowerCase() === 'class';
      });
  ...
  attr: function (node, attr)
    {
      var targetAttr = node.attrs.find(function (a)
      {
        return a.name === attr;
      });
...
var hasSelector = function (s, type)
  {
    ...
    return s.type === 'selector' || s.type === 'selectors' ? (s.nodes || []).some(function (n)
    {
      return hasSelector(n, type);
    }) : s.type === type;
  };
var dynamicFilters = {
  id: function (ast)
    {
      return (ast.inputs || []).some(function (i)
      {
        return i.name === 'id';
      });
    },
  ...
  class: function (ast)
    {
      return (ast.inputs || []).some(function (i)
      {
        return i.name === 'className' || i.name === 'ngClass';
      });
    }
};
          ...
          (ast.children || []).every(function (c)
          {
            return (ast instanceof compiler_1.ElementAst && _this.shouldVisit(c, strategies, selectorTypes)) ||
              (ast instanceof compiler_1.EmbeddedTemplateAst &&
                (ast.children || []).every(function (c)
                {
                  return _this.shouldVisit(c, strategies, selectorTypes);
                }));
          }));
      };
    ...
    CssVisitorCtrl.prototype.visitCssSelectorRule = function (ast)
      {
        var _this = this;
        try {
          var match = ast.selectors.some(function (s)
          {
            return _this.visitCssSelector(s);
          });
          if (!match) {
    ...
    CssVisitorCtrl.prototype.visitCssSelector = function (ast)
      {
        ...
        var strippedSelector = parts.map(function (s)
        {
    ...
    Walker.prototype.validateStyles = function (style, context, styleMetadata, baseStart)
      {
        ...
        if (isEncapsulationEnabled(encapsulation)) {
          style.visit(visitor);
          visitor.getFailures().forEach(function (f)
          {
            return _this.addFailure(f);
          });
        }
      };
    return Walker;
