...
if (pkg != '.') {
  packageDir = path.join(packageDir, 'node_modules', pkg);
}
...
browserify(bOpts)
  ...
  .bundle(function (err, buf)
  {
    ...
    if (result.map) {
      fs.writeFileSync(outputFile + '.min.js.map', result.map);
    }
    if (standalone) {
      fs.writeFileSync(outputFile + '.bundle.js', buf);
    }
    if (result.warnings) {
