...
function copy(from, to, opts)
  {
    ...
    if (opts.isWindows == null) {
      opts.isWindows = isWindows
    }
    if (!opts.Promise) {
    ...
    if (!opts.recurseWith) {
      opts.recurseWith = copyItem
    }
    if (!opts.lstat) {
      opts.lstat = promisify(opts.Promise, fs.lstat)
    }
    if (!opts.stat) {
      opts.stat = promisify(opts.Promise, fs.stat)
    }
    if (!opts.chown) {
      opts.chown = promisify(opts.Promise, fs.chown)
    }
    if (!opts.readdir) {
      opts.readdir = promisify(opts.Promise, fs.readdir)
    }
    if (!opts.readlink) {
      opts.readlink = promisify(opts.Promise, fs.readlink)
    }
    if (!opts.symlink) {
      opts.symlink = promisify(opts.Promise, fs.symlink)
    }
    if (!opts.chmod) {
      opts.chmod = promisify(opts.Promise, fs.chmod)
    }
